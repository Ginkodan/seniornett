...
export type ChatContactMessage = {
  id: string;
  senderId: string;
  role: "own" | "other";
  text: string;
...
function parseContactProfile(input: unknown): ChatContactProfile {
  const source = input && typeof input === "object" ? (input as Record<string, unknown>) : {};
  const tableChat = source.tableChat && typeof source.tableChat === "object"
    ? (source.tableChat as Record<string, unknown>)
    : {};
  const interestsRaw = tableChat.interests;

  return {
    location: typeof tableChat.location === "string" ? tableChat.location : "",
    age: typeof tableChat.age === "string" ? tableChat.age : "",
    interests: Array.isArray(interestsRaw) ? interestsRaw.filter((item): item is string => typeof item === "string") : [],
    note: typeof tableChat.note === "string" ? tableChat.note : "",
  };
}
...
export async function buildChatBootstrap(identity: RequestIdentity): Promise<ChatBootstrap> {
  ...
  for (const contact of allowedContacts) {
    ...
    contacts.push({
      ...
      profile: parseContactProfile(contact.profile),
...
export async function ensureChatRelationship(
  userId: string,
  contactUserId: string,
  label?: string | null
): Promise<void> {
  ...
  await getPool().query(
    `INSERT INTO chat_relationships (user_id, contact_user_id, label)
     ...
     DO UPDATE SET label = COALESCE(EXCLUDED.label, chat_relationships.label)`,
    [userId, contactUserId, label || null]
  );
}
